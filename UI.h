...
class UIClass {
public:
	UIClass();//构造函数   初始化类的时候执行
	~UIClass();//析构函数   类消亡的时候执行

	//void PrintUI(const char* exeName, const char* UIName, const char* color,
	//	short x, short y, WORD UIWide, WORD UIHigh);
private:
...
//void UIClass::PrintUI(const char* exeName, const char* UIName, const char* color, short x, short y, WORD UIWide, WORD UIHigh)
//{
...
//    if (freopen_s(&stdin, "conin$", "r+t", stdin) != 0) {
//        perror("freopen_s failed");
//        exit(1);
//    }
//    //if (freopen_s(stdout, "conout$", "w+t", stdout) != 0) {
//    //    perror("freopen_s failed");
//    //    exit(1);
//    //}
//
